...
func (s *SessionController) GetSessionCommandLogs(c *gin.Context) {
	...
	if sdkVersion != "" {
		upgrader.Subprotocols = []string{"X-Deck-SDK-Version~" + sdkVersion}
	} else {
	...
	if err != nil {
		log.Debugf("Failed to compare versions: %v", err)
		versionComparison = util.Pointer(1)
	}
	isCombinedOutput := (versionComparison != nil && *versionComparison < 0 && sdkVersion != "0.0.0-dev") || (sdkVersion == "" && c.Request.Header.Get("X-Deck-Split-Output") != "true")
		...
		ReadLog(c, logFile, util.ReadLogWithExitCode, exitCodeFilePath, func(conn *websocket.Conn, messages chan []byte, errors chan error) {
			...
			for {
				...
				case <-session.ctx.Done():
					...
					if err != nil {
						log.Errorf("Failed to write close control: %v", err)
					}
					conn.Close()
					return
				case msg := <-messages:
				...
				case <-errors:
					...
					if isCombinedOutput && len(buffer) > 0 {
						...
						if len(remainingData) > 0 {
							...
							if writeErr != nil {
								log.Errorf("Failed to write remaining data: %v", writeErr)
							}
						}
					}
...
func ReadLog[T any](ginCtx *gin.Context, logReader io.Reader, readFunc func(context.Context, io.Reader, bool, string, chan T, chan error), exitCodeFilePath string, wsWriteFunc func(*websocket.Conn, chan T, chan error)) {
	...
	defer func() {
		...
		if err != nil {
			log.Tracef("Failed to write close control: %v", err)
		}
		ws.Close()
	}()
