...
func (h *forwardedUnixHandler) HandleSSHRequest(ctx ssh.Context, _ *ssh.Server, req *gossh.Request) (bool, []byte) {
	log.WithField("session_id", ctx.SessionID()).Debug("handling SSH unix forward")
	conn, ok := ctx.Value(ssh.ContextKeyConn).(*gossh.ServerConn)
	if !ok {
		log.WithField("session_id", ctx.SessionID()).Warn("SSH unix forward request from client with no gossh connection")
		return false, nil
	}
	...
	case "streamlocal-forward@openssh.com":
		...
		if err != nil {
			log.WithField("session_id", ctx.SessionID()).WithError(err).Warn("parse streamlocal-forward@openssh.com request (SSH unix forward) payload from client")
			return false, nil
		}

		addr := reqPayload.SocketPath
		log.WithFields(log.Fields{"session_id": ctx.SessionID(), "socket": addr}).Debug("request begin SSH unix forward")
		...
		if ok {
			...
			// request.
			log.WithField("session_id", ctx.SessionID()).Warn("SSH unix forward request for socket path that is already being forwarded on this session, ignoring")
			return true, nil
		}
		...
		if err != nil {
			log.WithError(err).Error("Failed to create socket parent directory")
			return false, nil
		}
		...
		if err != nil && !errors.Is(err, fs.ErrNotExist) {
			log.WithField("session_id", ctx.SessionID()).WithError(err).Warn("remove existing socket for SSH unix forward request")
			return false, nil
		}
		...
		if err != nil {
			log.WithField("session_id", ctx.SessionID()).WithError(err).Warn("listen on Unix socket for SSH unix forward request")
			return false, nil
		}
		log.WithFields(log.Fields{"session_id": ctx.SessionID(), "socket": addr}).Debug("SSH unix forward listening on socket")
		...
		h.forwards[key] = ln
		h.Unlock()
		log.WithField("session_id", ctx.SessionID()).Debug("SSH unix forward added to cache")
		...
		go func() {
			...
			for {
				...
				if err != nil {
					if !errors.Is(err, net.ErrClosed) {
						log.WithField("session_id", ctx.Value(ssh.ContextKeySessionID)).WithError(err).Warn("accept on local Unix socket for SSH unix forward request")
					}
					// closed below
					log.WithField("session_id", ctx.Value(ssh.ContextKeySessionID)).Debug("SSH unix forward listener closed")
					break
				}
				log.WithField("session_id", ctx.Value(ssh.ContextKeySessionID)).Debug("accepted SSH unix forward connection")
				...
				go func() {
					...
					if err != nil {
						log.WithField("session_id", ctx.Value(ssh.ContextKeySessionID)).WithError(err).Warn("open SSH unix forward channel to client")
						_ = c.Close()
						return
					}
			...
			h.Unlock()
			log.WithField("session_id", ctx.Value(ssh.ContextKeySessionID)).Debug("SSH unix forward listener removed from cache")
			_ = ln.Close()
		}()
	...
	case "cancel-streamlocal-forward@openssh.com":
		...
		err := gossh.Unmarshal(req.Payload, &reqPayload)
		if err != nil {
			log.WithField("session_id", ctx.SessionID()).WithError(err).Warn("parse cancel-streamlocal-forward@openssh.com (SSH unix forward) request payload from client")
			return false, nil
		}
		log.WithFields(log.Fields{"session_id": ctx.SessionID(), "socket": reqPayload.SocketPath}).Debug("request to cancel SSH unix forward")
		...
		h.Unlock()
		if !ok {
			log.WithField("session_id", ctx.SessionID()).Warn("SSH unix forward not found in cache")
			return true, nil
		}
		_ = ln.Close()
		return true, nil
