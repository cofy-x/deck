...
func (m *ComputerUseRPCClient) OpenBrowser(request *BrowserOpenRequest) (*Empty, error) {
	err := m.client.Call("Plugin.OpenBrowser", request, new(Empty))
	return new(Empty), err
}

func (m *ComputerUseRPCClient) CloseBrowser() (*Empty, error) {
	err := m.client.Call("Plugin.CloseBrowser", new(any), new(Empty))
	return new(Empty), err
}
