...
This package provides a Computer Use plugin used by the Deck Daemon to allow agents to control VNC desktop environments.
...
```dockerfile
# Setup VNC
RUN mkdir -p /home/deck/.vnc && \
    chown -R deck:deck /home/deck/.vnc
...
```dockerfile
# Create the Deck user and configure sudo access
RUN useradd -m deck && echo "deck ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/91-deck
...
- `VNC_USER`: User to run VNC processes (default: "deck")
...
Log files are stored in `~/.deck/computeruse/`:
