...
func (u *ComputerUse) TakeCompressedScreenshot(req *api.CompressedScreenshotRequest) (*api.ScreenshotResponse, error) {
	...
	response := &api.ScreenshotResponse{
		Screenshot: base64Str,
		SizeBytes:  len(imageData),
	}

	if req.ShowCursor {
		response.CursorPosition = &api.Position{
...
func (u *ComputerUse) TakeCompressedRegionScreenshot(req *api.CompressedRegionScreenshotRequest) (*api.ScreenshotResponse, error) {
	params := ImageCompressionParams{
		Format:  req.Format,
		Quality: req.Quality,
		Scale:   req.Scale,
...
func (u *ComputerUse) GetDisplayInfo() (*api.DisplayInfoResponse, error) {
	n := screenshot.NumActiveDisplays()
	displays := make([]api.DisplayInfo, n)

	for i := 0; i < n; i++ {
		bounds := screenshot.GetDisplayBounds(i)
		displays[i] = api.DisplayInfo{
			ID: i,
	...
	return &api.DisplayInfoResponse{
		Displays: displays,
	}, nil
}

func (u *ComputerUse) GetWindows() (*api.WindowsResponse, error) {
	...
	for _, id := range titles {
		...
		if title != "" {
			...
			windows = append(windows, api.WindowInfo{
				ID:    id,
				Title: title,
				Position: api.Position{
