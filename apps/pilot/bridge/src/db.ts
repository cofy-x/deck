...
export class BridgeStore {
  ...
  deleteSession(channel: ChannelName, peerId: string): boolean {
    const stmt = this.db.prepare(
      'DELETE FROM sessions WHERE channel = ? AND peer_id = ?',
    );
  ...
  findPairingRequestsByCode(code: string): PairingRow[] {
    const now = Date.now();
    const stmt = this.db.prepare(
      'SELECT channel, peer_id, code, created_at, expires_at FROM pairing_requests WHERE code = ? AND expires_at > ? ORDER BY created_at ASC',
    );
    return stmt.all(code, now) as PairingRow[];
  }
  ...
  approvePairingRequest(channel: ChannelName, code: string): PairingRow | null {
    ...
    const del = this.db.prepare(
      'DELETE FROM pairing_requests WHERE channel = ? AND peer_id = ?',
    );
