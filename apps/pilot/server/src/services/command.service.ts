...
import type { CommandItem, JsonObject } from '../types/index.js';
import { parseFrontmatter, buildFrontmatter } from '../utils/frontmatter.js';
import { exists } from '../utils/fs.js';
import { projectCommandsDir } from '../utils/workspace-files.js';
import {
  validateCommandName,
  sanitizeCommandName,
...
export interface UpsertCommandPayload {
  name: string;
  description?: string;
  template: string;
  agent?: string;
...
async function listCommandsInDir(
  ...
  for (const entry of entries) {
    ...
    items.push({
      ...
      subtask:
        typeof data['subtask'] === 'boolean' ? data['subtask'] : undefined,
      scope,
    });
  }
  return items;
...
export async function upsertCommand(
  ...
  const frontmatterData: JsonObject = {
    name: sanitized,
