...
import type { JsonObject, PluginItem } from '../types/index.js';
...
import { validatePluginSpec } from '../utils/validators.js';
...
function pluginListFromConfig(config: JsonObject): string[] {
  const plugin = config['plugin'];
  if (typeof plugin === 'string') return [plugin];
  if (Array.isArray(plugin))
    return plugin.filter((item): item is string => typeof item === 'string');
  return [];
}
...
export async function addPlugin(
  ...
  const existing = pluginSpecs.find(
...
export async function removePlugin(
  ...
  const filtered = pluginSpecs.filter(
    (item) => normalizePluginSpec(item) !== normalized,
  );
